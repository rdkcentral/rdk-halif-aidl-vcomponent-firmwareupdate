...
VCOMPONENT="${VCOMPONENT:-firmwareupdate}"
VCOMPONENT_VERSION="${VCOMPONENT_VERSION:-0.2.0.0}"

declare -A HALIF_COMPONENTS=(
  [firmwareupdate]="current"
)

OUT_TAR_FILE=vcomponent-FirmwareUpdate.tar.gz
...
declare -r RDK_HAL_DIR=${TOP}/rdk-halif-aidl
...
generate_hal_interfaces()
{
    ...
    # Enter the HAL repository and build the required tools/modules before
    ...
    ./build_modules.sh "${VCOMPONENT}" --version "${VCOMPONENT_VERSION}" || { echo -e "${BOLD}* ${RED}Failed to build HALIF modules. Exiting.${RESET}"; exit 1; }
    popd >/dev/null

    # After generating HAL interfaces/tools, path for the derived environment variables
    # so subsequent build steps (and developers) can reuse consistent paths.
    halif_binder_env
}
...
#
# This intentionally derives paths from the checked-out HALIF repo root to avoid
# hardcoding absolute host paths. If a path layout changes in the HALIF repo,
# adjust the small set of *_REL defaults below (or override vars before calling).
###############################################################################
halif_binder_env()
{
    # ----------------------------
    # Component selection (generic)
    # ----------------------------
    # Keep these in one place so it's easy to adapt this build script for other
    # components later (or make it parameterized).
    AIDL_SRC_VERSION="${AIDL_SRC_VERSION:-${VCOMPONENT_VERSION}}"

    # Component headers (generic)
    VCOMPONENT_HALIF_INCLUDE_DIR="${VCOMPONENT_HALIF_INCLUDE_DIR:-${RDK_HAL_DIR}/${VCOMPONENT}/${VCOMPONENT_VERSION}/include}"

    # ----------------------------
    # Binder toolchain + SDK (prefer HALIF build outputs)
    # ----------------------------
    # build_binder.sh/build_modules.sh stage tools/runtime under out/target and
    # reorganized Binder SDK headers under out/build/include/binder_sdk.
    #
    # AIDL compiler tools
    AIDL_BIN="${AIDL_BIN:-${RDK_HAL_DIR}/build/binder/aidl}"
    AIDL_CPP_BIN="${AIDL_CPP_BIN:-${RDK_HAL_DIR}/build/binder/aidl-cpp}"

    # Binder SDK for linking (target)
    ...
    # ----------------------------
    # HALIF outputs (libs + general includes)
    # ----------------------------
    # These paths follow the conventions used by the HALIF build scripts; keep
    # them overrideable because output layout may vary by branch/version.
    ...
    # PATH + runtime libs
    # ----------------------------
    # Prepend the binder toolchain bins; avoid duplicating if function is called
    ...
    # LD_LIBRARY_PATH is optional; extend it safely even if unset.
...
build_ut_core()
{
    ...
    # get the latest version of ut-core
...
install_linux_binder()
{
    ...
    #install -t ${BUILD_INSTALL_PREFIX}/bin ${BINDER_SDK_DIR}/bin/*
    install -t ${BUILD_INSTALL_PREFIX}/bin ${LINUX_BINDER_OUT_DIR}/bin/*
    #install -t ${BUILD_INSTALL_PREFIX}/lib ${BINDER_SDK_DIR}/lib/binder/*
...
###############################################################################
# Function to copy common directory from HALIF repo
###############################################################################
prepare_common_dir()
{
    ...
    # Copy fresh
    cp -r "${SRC_COMMON_DIR}" "${DEST_COMMON_DIR}"

    if [ $? -ne 0 ]; then
        echo -e "${RED}Failed to copy common directory!${RESET}"
        exit 1
    fi

    # Common directory path
    COMMON_INCLUDE_DIR="${DEST_COMMON_DIR}"

    echo -e "${GREEN}COMMON_INCLUDE_DIR=${COMMON_INCLUDE_DIR}${RESET}"
}

###############################################################################
# Function to build DeepSleep component
###############################################################################
build_FirmwareUpdate()
{
    cmake -S . -B build \
        ...
        -DFIRMWAREUPDATE_HALIF_INCLUDE_DIR="${VCOMPONENT_HALIF_INCLUDE_DIR}" \
        -DFIRMWAREUPDATE_HALIF_VERSION="${FIRMWAREUPDATE_HALIF_VERSION}" \
    ...
    if [ ${cfg_rc} -ne 0 ]; then
     ...
     if [ ${build_rc} -ne 0 ]; then
          echo "❌ Build failed (exit ${build_rc}). Not running install."
          return ${build_rc}
     fi
     ...
     # Copy HALIF-generated firmwareupdate shared library into this vcomponent build output.
     cp -f "${HALIF_LIB_DIR}/libfirmwareupdate-v${VCOMPONENT_VERSION}-cpp.so" "${TOP_BUILD_DIR}/" 2>/dev/null
}
...
echo -e "${BOLD}${GREEN}VTS Version            : ${RESET}${YELLOW}$(git rev-parse --abbrev-ref HEAD)${RESET}"
